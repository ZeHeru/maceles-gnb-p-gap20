...
from mace.modules.radial import ZBLBasis, GNBBasis
...
class MACE(torch.nn.Module):
    def __init__(
        ...
        if use_gnb:
            self.gnb_fn = GNBBasis(cutoff_lr=self.cutoff_lr, p=num_polynomial_cutoff)
            self.use_gnb = True
        if not use_so3:
    ...
    def forward(
        ...
        if hasattr(self, "use_gnb"):
            gnb_node_energy = self.gnb_fn(
                lengths, data["node_attrs"], data["edge_index"], self.atomic_numbers
            )
            gnb_energy = scatter_sum(
                src=gnb_node_energy, index=data["batch"], dim=-1, dim_size=num_graphs
            )  # [n_graphs,]
        else:
            pair_node_energy = torch.zeros_like(node_e0)
            pair_energy = torch.zeros_like(e0)
            gnb_node_energy = torch.zeros_like(node_e0)
            gnb_energy = torch.zeros_like(e0)
        ...
        # Interactions
        energies = [e0, pair_energy, gnb_energy]
        node_energies_list = [node_e0, pair_node_energy, gnb_node_energy]
...
class ScaleShiftMACE(MACE):
    ...
    def forward(
        ...
        # GNB potential (uses long-range neighbor list if available)
        if hasattr(self, "use_gnb"):
            if lengths_lr is not None and data.get("edge_index_lr") is not None:
                gnb_node_energy = self.gnb_fn(
                    lengths_lr, data["node_attrs"], data["edge_index_lr"], self.atomic_numbers
                )
            else:
                gnb_node_energy = self.gnb_fn(
                    lengths, data["node_attrs"], data["edge_index"], self.atomic_numbers
        ...
        # Interactions
        node_es_list = [pair_node_energy, gnb_node_energy]
