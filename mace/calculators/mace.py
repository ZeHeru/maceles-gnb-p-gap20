...
class MACECalculator(Calculator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Models trained with --use_gnb / long-range extensions store a
        # cutoff_lr attribute. The forward pass consumes edge_index_lr built
        # with this cutoff; if we do not propagate it into AtomicData the GNB
        # branch silently falls back to the short-range neighbor list,
        # producing a density-dependent energy shift (forces are barely
        # affected because GNB force ~ r^-7 is short-range dominated).
        cutoff_lrs = []
        for m in self.models:
            c = getattr(m, "cutoff_lr", None)
            if c is None:
                continue
            cutoff_lrs.append(float(c.cpu() if torch.is_tensor(c) else c))
        if cutoff_lrs:
            if not all(abs(c - cutoff_lrs[0]) < 1e-6 for c in cutoff_lrs):
                raise ValueError(f"committee cutoff_lr mismatch: {cutoff_lrs}")
            self.cutoff_lr = cutoff_lrs[0]
        else:
            self.cutoff_lr = None
