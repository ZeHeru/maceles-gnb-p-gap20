...
class AtomicData(torch_geometric.data.Data):
    ...
    def __init__(
        ...
        edge_index_lr: Optional[torch.Tensor] = None,
        shifts_lr: Optional[torch.Tensor] = None,
        unit_shifts_lr: Optional[torch.Tensor] = None,
    ...
    def from_config(
        ...
        **kwargs,  # pylint: disable=unused-argument
        ...
        # )
        edge_index, shifts, unit_shifts, cell, edge_index_lr, shifts_lr, unit_shifts_lr = get_neighborhood(
            positions=config.positions,
            cutoff=cutoff,
            cutoff_lr=cutoff_lr, 
            pbc=deepcopy(config.pbc),
        ...
        cls_kwargs = dict(
            ...
            edge_index_lr=torch.tensor(edge_index_lr, dtype=torch.long) if edge_index_lr is not None else None,
            shifts_lr=torch.tensor(shifts_lr, dtype=torch.get_default_dtype()) if shifts_lr is not None else None,
            unit_shifts=torch.tensor(unit_shifts, dtype=torch.get_default_dtype()),
            unit_shifts_lr=torch.tensor(unit_shifts_lr, dtype=torch.get_default_dtype()) if unit_shifts_lr is not None else None,
            cell=cell,
            node_attrs=one_hot,
            weight=weight,
            head=head,
