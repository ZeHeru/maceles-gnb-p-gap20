...
def run(args: argparse.Namespace) -> None:
    ...
    # Models trained with --use_gnb store cutoff_lr; the GNB forward branch
    # consumes edge_index_lr built with that cutoff. If we don't propagate it
    # here, GNB silently falls back to the short-range edge list and produces
    # a density-dependent per-atom energy shift.
    _c = getattr(model, "cutoff_lr", None)
    if _c is not None and torch.is_tensor(_c):
        _c = float(_c.cpu())
    elif _c is not None:
        _c = float(_c)
